...
/**
 * A `D1Database` backed by Cloudflare's HTTP API rather than a Worker binding.
 *
 ...
 * `prepare().bind().all()`, which is the entire surface `kysely-d1` uses, and
 * claims the full `D1Database` type on that basis. Every other method is absent,
 * so this is migration plumbing, not a general-purpose remote D1 client.
 */
 ...
 *
 * Carries the same names-only introspector {@link d1MigrationDialect} adds, which
 * the Migrator needs on D1 whichever transport it reaches the database through.
 */
