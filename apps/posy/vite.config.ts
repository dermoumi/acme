...
function nodeBuild(isSsrBuild: boolean) {
  ...
  return {
    ...
    rolldownOptions: {
      input: {
        index: "src/server/index.node.ts",
        migrate: "src/server/migrate.node.ts",
      },
...
export default defineConfig(({ mode, isSsrBuild }) => ({
  ...
  // A native binding cannot be bundled; inlining the rest is what lets the
  // runtime image ship without a node_modules tree.
