...
describe("scrubEvent", () => {
  it("keeps the request body, masking only sensitive keys", () => {
    const { request } = scrub();
  ...
  it("keeps harmless query params, masking sensitive ones", () => {
    const { request } = scrub();
  ...
  it("drops sensitive headers whatever their casing, keeps the rest", () => {
    const { request } = scrub();
  ...
  it("masks a whole nested object when its own key is sensitive", () => {
    const event: ErrorEvent = {
      type: undefined,
      request: {
  ...
  it("key matching ignores case on both sides", () => {
    ...
    for (const [field, key] of cases) {
      expect(body(field, key), `field ${field} / key ${key}`).not.toContain(
        NOTE,
      );
    }
  ...
  it("redactKeys marks project specific fields as sensitive", () => {
  ...
  it("leaves extra and tags untouched", () => {
  ...
  it("passes through an event carrying no request", () => {
    const event: ErrorEvent = { type: undefined, message: "no request here" };
    expect(scrub(event)).toEqual(event);
  });

  // Unparseable means unmaskable, so the level decides:
  // light keeps the body for debugging, full withholds it.
  it("withholds a body it cannot parse, unless masking is light", () => {
    const event: ErrorEvent = {
      type: undefined,
  ...
  it("masks a form-encoded body by key", () => {
  ...
  it("redactKeys drops matching headers, not just body keys", () => {
    const event = {
      type: undefined,
      request: { headers: { "X-Tenant-Id": "acme", accept: "*/*" } },
    } as ErrorEvent;

    expect(scrub(event, ["tenant"]).request?.headers).toEqual({
      accept: "*/*",
    });
